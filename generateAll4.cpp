namespace matrixInit {
	...
	void getAllMatrices(MatrixCollection &vec) {
	...
	bool generateAllToFile() {
		...
		vecfiltered.saveToFile("output.mt");
		return true;
	}

	MatrixCollection readAllFromFile(std::string& s) {
		MatrixCollection newmc = MatrixCollection::readFromFile(s);
		std::cout << newmc.size() << "\n";
		return newmc;
	}
}
